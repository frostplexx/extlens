/**
 * Bridge lifecycle, plus the two pieces of state the main process pushes.
 *
...
import { createBridge, type Bridge, type BridgeStatus } from "../bridge";
...
export function useBridge(): BridgeHandle {
    ...
    if (!bridgeRef.current) bridgeRef.current = createBridge(setStatus);
